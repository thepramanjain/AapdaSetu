...
class DisasterState(TypedDict, total=False):
    ...
    # RWA Community Response (Phase 1)
    rwa_community: dict
    
    # Heatwave Prediction (Phase 3)
    heatwave_prediction: dict
...
def rwa_community_node(state: DisasterState) -> DisasterState:
    """LangGraph node: RWA Community Response Agent."""
    app_logger.info("LangGraph Node: Starting RWA Community Agent...")
    rwa_agent = RWAAgent()
    result = rwa_agent.run(state)
    completed = state.get("completed_agents", []) + ["rwa_community"]
    return {
        **state,
        "rwa_community": result.data,
        "completed_agents": completed
    }


def heatwave_prediction_node(state: DisasterState) -> DisasterState:
    """LangGraph node: Heatwave Prediction Agent."""
    app_logger.info("LangGraph Node: Starting Heatwave Prediction Agent...")
    hw_agent = HeatwaveAgent()
    result = hw_agent.run(state)
    completed = state.get("completed_agents", []) + ["heatwave_prediction"]
    return {
        **state,
        "heatwave_prediction": result.data,
        "completed_agents": completed
    }
...
def build_disaster_graph():
    ...
    workflow.add_node("rwa_community", rwa_community_node)
    workflow.add_node("heatwave_prediction", heatwave_prediction_node)
...
def execute_pipeline(query: str) -> dict:
    ...
    if not disaster or disaster.lower() not in ["flood", "earthquake", "heatwave"]:
        return {
            "error": "Missing/Unknown Disaster",
            "message": "I couldn't detect a supported disaster type (like Flood, Earthquake, or Heatwave). Could you please specify?"
        }
