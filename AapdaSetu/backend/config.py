...
class Settings(BaseSettings):
    ...
    # Shardeum Blockchain (Phase 2)
    SHARDEUM_RPC_URL: str = Field(default="", description="Shardeum Testnet RPC endpoint")
    SHARDEUM_PRIVATE_KEY: str = Field(default="", description="Private key for Shardeum wallet (never logged)")
    SHARDEUM_CONTRACT_ADDRESS: str = Field(default="", description="Deployed contract address on Shardeum")
    SHARDEUM_CHAIN_ID: int = Field(default=0, description="Shardeum Testnet chain ID")
    SHARDEUM_EXPLORER_URL: str = Field(default="", description="Shardeum block explorer base URL")

    # Open-Meteo (Phase 3 — no API key required)
    OPEN_METEO_BASE_URL: str = Field(default="https://api.open-meteo.com/v1", description="Open-Meteo API base URL")

    # RWA Module (Phase 1)
    RWA_DEFAULT_RADIUS_KM: float = Field(default=25.0, description="Default geo-search radius for RWA societies (km)")
