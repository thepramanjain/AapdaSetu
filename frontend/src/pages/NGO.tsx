...
export const NGO: React.FC = () => {
  const [activeTab, setActiveTab] = useState<'all' | 'ai' | 'ledger' | 'inventory' | 'impact'>('all');
  ...
  return (
    <div className="min-h-screen pb-24 pt-28 relative bg-slate-50/50 text-slate-800">
      ...
      <div className="mx-auto max-w-7xl px-4 sm:px-6 space-y-8">
        ...
        {/* ─── MIDDLE NEUMORPHIC TAB NAVBAR ─── */}
        <div className="flex items-center justify-center pt-2 pb-2">
          <div
            className="inline-flex p-1.5 rounded-2xl gap-2 max-w-full overflow-x-auto"
            style={{
          ...
          >
            {[
              ...
              { id: 'ai', label: 'AI Proposals & Wizard', icon: Sparkles },
              { id: 'ledger', label: 'File Movement & Sanctions', icon: FileText },
              { id: 'inventory', label: 'Live Inventory & Supplies', icon: Truck },
              { id: 'impact', label: 'Verified Impact Metrics', icon: BarChart3 },
            ].map((tab) => {
              const Icon = tab.icon;
              ...
              return (
                <button
                  ...
                  className={clsx(
                    ...
                    isActive
                      ? 'text-emerald-900 bg-white shadow-md'
                ...
                >
                  <Icon className={clsx('h-4 w-4', isActive ? 'text-emerald-600' : 'text-slate-500')} />
                  <span>{tab.label}</span>
                </button>
              );
            })}
